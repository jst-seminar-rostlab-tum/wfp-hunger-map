...
export function middleware(request: NextRequest) {
  ...
  const cspHeader = `
    ...
    connect-src 'self' https://region1.google-analytics.com https://static.hungermapdata.org https://cdn.hungermapdata.org ${process.env.NEXT_PUBLIC_API_URL?.slice(0, -3)} ${process.env.NEXT_PUBLIC_V3_API_URL?.slice(0, -3)} ${process.env.NEXT_PUBLIC_CHATBOT_API_URL ?? ''} ${process.env.NEXT_PUBLIC_EMAIL_SERVICE ?? ''};
